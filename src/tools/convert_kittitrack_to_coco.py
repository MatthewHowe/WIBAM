...
if __name__ == "__main__":
    for split in SPLITS:
        ...
        for i in VIDEO_SETS[split]:
            ...
            video_path = DATA_PATH + "/data_tracking_image_2/{}ing/image_02/{}".format(
                ann_dir, video_name
            )
            calib_path = (
                DATA_PATH
                + "data_tracking_calib/{}ing/calib/".format(ann_dir)
            ...
            if CREATE_HALF_LABEL and "half" in split:
                image_range = (
                    [0, num_images_video // 2 - 1]
                    if split == "train_half"
                    else [num_images_video // 2, num_images_video - 1]
                )
            else:
            ...
            for j, image_name in enumerate(image_files):
                ...
                num_images += 1
                image_info = {
                    "file_name": "{}/{:06d}.png".format(video_name, j),
                    "id": num_images,
                    "calib": calib.tolist(),
                    "video_id": i + 1,
            ...
            for ann_ind, txt in enumerate(anns):
                ...
                amodel_center = project_to_image(
                    np.array(
                        [location[0], location[1] - dim[0] / 2, location[2]], np.float32
                    ).reshape(1, 3),
                    calib,
                )[0].tolist()
                ann = {
                    "image_id": frame_id + 1 - image_range[0] + image_id_base,
                ...
                if CREATE_HALF_LABEL and "half" in split:
                    ...
                    label_out_file.write(
                        "{} {}".format(out_frame_id, txt[txt.find(" ") + 1 :])
                    )
