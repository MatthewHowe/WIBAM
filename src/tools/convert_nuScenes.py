...
def main():
    ...
    for split in SPLITS:
        ...
        for sample in nusc.sample:
            ...
            if not (split in ["mini", "test"]) and not (
                scene_name in SCENE_SPLITS[split]
            ):
                continue
            if sample["prev"] == "":
                ...
                if sensor_name in USED_SENSOR:
                    ...
                    for i in range(len(anns)):
                        ...
                        for j in range(len(anns)):
                            if anns[i]["depth"] - min(anns[i]["dim"]) / 2 > anns[j][
                                "depth"
                            ] + max(anns[j]["dim"]) / 2 and _bbox_inside(
                    ...
                    if DEBUG:
                        ...
                        for ann in visable_anns:
                            ...
                            pt_3d = unproject_2d_to_3d(
                                ann["amodel_center"], ann["depth"], calib
                            )
                            pt_3d[1] += ann["dim"][0] / 2
