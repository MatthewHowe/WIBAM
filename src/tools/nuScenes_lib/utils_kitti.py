...
class KittiDB:
    ...
    def __init__(
        self, root: str = "/data/sets/kitti", splits: Tuple[str, ...] = ("train",)
    ):
        """
    ...
    def box_nuscenes_to_kitti(
        ...
        r0_rect: Quaternion,
        kitti_to_nu_lidar_inv: Quaternion = Quaternion(
            axis=(0, 0, 1), angle=np.pi / 2
        ).inverse,
    ) -> Box:
        """
    ...
    def get_boxes(
        self, token: str, filter_classes: List[str] = None, max_dist: float = None
    ) -> List[Box]:
        """
    ...
    def project_pts_to_image(
        self, pointcloud: LidarPointCloud, token: str
    ) -> np.ndarray:
        """
    ...
    def render_sample_data(
        ...
        if sensor_modality == "lidar":
            ...
            pc.rotate(
                self.kitti_to_nu_lidar.rotation_matrix
            )  # In nuScenes lidar frame.
            # Alternative options:
            # depth = pc.points[1, :]
            # height = pc.points[2, :]
