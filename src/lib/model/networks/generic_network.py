...
backbone_factory = {"dla34": dla34, "resnet": Resnet, "mobilenet": MobileNetV2}

neck_factory = {"dlaup": DLASeg, "msraup": MSRAUp}
...
class GenericNetwork(nn.Module):
    def __init__(self, num_layers, heads, head_convs, num_stacks=1, opt=None):
        ...
        print(
            "Using generic model with backbone {} and neck {}".format(
        ...
        for head in self.heads:
            ...
            if len(head_conv) > 0:
                ...
                for k in range(1, len(head_conv)):
                    convs.append(
                        nn.Conv2d(
                            head_conv[k - 1], head_conv[k], kernel_size=1, bias=True
                        )
                    )
                if len(convs) == 1:
                ...
                fc = nn.Conv2d(
                    last_channel, classes, kernel_size=1, stride=1, padding=0, bias=True
                )
                if "hm" in head:
