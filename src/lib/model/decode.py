...
from .utils import _gather_feat, _nms, _topk, _topk_channel, _tranpose_and_gather_feat
...
def generic_decode(output, K=100, opt=None):
    ...
    for head in regression_heads:
        if head in output:
            ret[head] = _tranpose_and_gather_feat(output[head], inds).view(batch, K, -1)
    ...
    if "ltrb_amodal" in output:
        ...
        bboxes_amodal = torch.cat(
            [
                xs0.view(batch, K, 1) + ltrb_amodal[..., 0:1],
                ys0.view(batch, K, 1) + ltrb_amodal[..., 1:2],
                xs0.view(batch, K, 1) + ltrb_amodal[..., 2:3],
                ys0.view(batch, K, 1) + ltrb_amodal[..., 3:4],
            ],
            dim=2,
        )
