...
from model.losses import BinRotLoss, FastFocalLoss, RegWeightedL1Loss, WeightedBCELoss
from model.mv_losses import ReprojectionLoss
from model.utils import _sigmoid, flip_lr, flip_lr_off, flip_tensor
import numpy as np
from progress.bar import Bar
import torch
from utils.debugger import Debugger
from utils.post_process import generic_post_process
from utils.utils import AverageMeter, Profiler
    ...
    def forward(self, outputs, batch):
        ...
        for head in regression_heads:
            if head in output:
                losses[head] += (
                    self.crit_reg(
                        output[head], batch[head + "_mask"], batch["ind"], batch[head]
                    )
                    / opt.num_stacks
                )
...
class Trainer(object):
    def __init__(self, opt, model, writer, total_writer, optimizer=None, dataset=None):
        self.opt = opt
    ...
    def run_epoch(self, phase, epoch, data_loader):
        ...
        for iter_id, batch in enumerate(data_loader):
            ...
            for l in loss_stats:
                ...
                if loss_val != 0:
                    avg_loss_stats[l].update(loss_val, batch["image"].size(0))
                    ...
                    if l == "amodel_offset":
                        ...
                        if phase == "train":
                            self.writer.add_scalar(
                                "{}: {}_{}".format(self.dataset, l, phase),
                                avg_loss_stats[l].val,
                                self.total_steps_train,
                            )
                        elif phase == "val":
                            self.writer.add_scalar(
                                "{}: {}_{}".format(self.dataset, l, phase),
                                avg_loss_stats[l].val,
                                self.total_steps_val,
                            )

            for l, val in avg_loss_stats.items():
                Bar.suffix = Bar.suffix + "|{} {:.2f} ".format(l, avg_loss_stats[l].avg)
            Bar.suffix = Bar.suffix + "Data {dt.avg:.3f}s |Net {bt.avg:.3f}s".format(
        ...
        for loss, value in avg_loss_stats.items():
            self.total_writer.add_scalar(
                "EPOCH AV {}: {}_{}".format(self.dataset, phase, loss), value.avg, epoch
            )
