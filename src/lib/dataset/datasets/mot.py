...
class MOT(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        img_dir = os.path.join(
            data_dir, "{}".format("test" if "test" in self.dataset_version else "train")
        )
    ...
    def save_results(self, results, save_dir):
        results_dir = os.path.join(
            save_dir, "results_mot{}".format(self.dataset_version)
        )
    ...
    def run_eval(self, results, save_dir):
        ...
        gt_type_str = "{}".format(
            "_train_half"
            if "17halftrain" in self.opt.dataset_version
            else "_val_half"
            if "17halfval" in self.opt.dataset_version
            else ""
        )
