...
class WIBAM(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        if opt.dataset_version == "":
            ann_path = os.path.join(data_dir, "annotations", "wibam_all.json").format(
                split, opt.dataset_version
            )
        else:
            ann_path = os.path.join(data_dir, "annotations", "wibam_{}_{}.json").format(
                split, opt.dataset_version
            )
    ...
    def __getitem__(self, index):
        ...
        imgs, cams, imgs_anns, imgs_info, imgs_path, drawing_images = self._load_data(
            index
        )
    ...
    def _add_instance(
        ...
        ct = np.array(
            [(bbox_hm[0] + bbox_hm[2]) / 2, (bbox_hm[1] + bbox_hm[3]) / 2],
            dtype=np.float32,
        )
    ...
    def _init_ret(self, ret, gt_det, num_cams):
        ...
        regression_head_dims = {"reg": 2, "wh": 2}
    ...
    def _load_data(self, index):
        if self.instance_batching:
            ...
            id = self.instances[index]
        img, cams, anns, img_info, img_path, drawing_images = self._load_image_anns(
            id, self.coco, self.img_dir
        )
...
class WIBAM_test(Dataset):
    mean = np.array([0.40789655, 0.44719303, 0.47026116], dtype=np.float32).reshape(
        1, 1, 3
    )
    std = np.array([0.2886383, 0.27408165, 0.27809834], dtype=np.float32).reshape(
        1, 1, 3
    )
                ...
                for filename in filenames:
                    self.paths[idx] = {
                        "image_path": os.path.join(
                            self.images_dir, cam, str(filename) + ".jpg"
                        ),
    ...
    def __getitem__(self, index):
        ...
        center = np.array(
            [image.shape[1] / 2.0, image.shape[0] / 2.0], dtype=np.float32
        )
        trans_input = get_affine_transform(
    ...
    def get_annotations(self, index):
        ...
        calib_dir = os.path.join(self.data_dir, "calib/calibration_{}.npz".format(cam))
