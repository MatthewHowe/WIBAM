...
from .ddd_utils import compute_box_3d, draw_box_3d, project_to_image
...
class Debugger(object):
    ...
    def add_coco_bbox(
        self, bbox, cat, conf=1, show_txt=True, no_bbox=False, img_id="default"
    ):
        ...
        if self.opt.only_show_dots:
            ...
            cv2.circle(self.imgs[img_id], ct, 8, c, -1, lineType=cv2.LINE_AA)
    ...
    def add_coco_hp(self, points, tracking_id=0, img_id="default"):
        ...
        for j, e in enumerate(self.edges):
            if (
                points[e].min() > 0
                and points[e, 0].max() < w
                and points[e, 1].max() < h
            ):
                c = (
                    self.ec[j]
                    if not self.opt.show_track_color
                    else self.track_color[tracking_id]
                )
                cv2.line(
                    self.imgs[img_id],
        ...
        if 1:
            ...
            if not self.with_3d:
                ...
                Xb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][
                    0
                ].flatten() + 0.5 * (self.xmax + self.xmin)
                Yb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][
                    1
                ].flatten() + 0.5 * (self.ymax + self.ymin)
                Zb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][
                    2
                ].flatten() + 0.5 * (self.zmax + self.zmin)
    ...
    def add_3d_detection(
        ...
        for item in dets:
            if (
                item["score"] > vis_thresh
                and "dim" in item
                and "loc" in item
                and "rot_y" in item
            ):
                cl = (
                    (self.colors[int(item["class"]) - 1, 0, 0]).tolist()
                    if not self.opt.show_track_color
                    else self.track_color[int(item["tracking_id"])]
                )
                ...
                if loc[2] > 1:
                    ...
                    if self.opt.show_track_color or self.opt.qualitative:
                        ...
                        self.add_coco_bbox(
                            ...
                            item["class"] - 1,
                            sc,
                            show_txt=not self.opt.not_show_txt,
                            no_bbox=True,
                            img_id=img_id,
                        )
                    if self.opt.show_track_color:
    ...
    def compose_vis_ddd(
        self, img_path, flipped, dets, calib, vis_thresh, pred, bev, img_id="out"
    ):
