...
from utils.bbox_iou_evaluation import bbox_iou, calculate_3D_iou, match_bboxes
from utils.ddd_utils import ddd2locrot, draw_box_3d, project_3d_bbox
from utils.drawing_utils import (
    BEV_image,
    draw_birds_eye,
    draw_results,
    initialise_birds_eye_image,
)
...
def test_calculations(batch, calib):
    ...
    center = cv2.projectPoints(
        location_wcf.reshape((1, 1, 3)), rvec, tvec, P, dist_coefs
    )[0]
...
def det_cam_to_det_3D_ccf(model_detections, calib):
    ...
    locations = unproject_2d_to_3d(
        model_detections["center"], model_detections["depth"], calib["P_det"]
    )
...
def dets_3D_wcf_to_dets_2D(detections, calib):
    ...
    for batch in range(BN):
        for cam in range(num_cams):
            ...
            bounding_box_cam = torch.div(
                bounding_box_cam[:], bounding_box_cam[:, 2][:, None, :]
            )
...
def det_3D_to_BBox_3D(detections, calib):
    ...
    for batch in range(BN):
        ...
        rotation_object = torch.where(
            rotation_object > np.pi, rotation_object - 2 * np.pi, rotation_object
        )
        rotation_object = torch.where(
            rotation_object < -np.pi, rotation_object + 2 * np.pi, rotation_object
        )
...
def decode_output(output, K=100, opt=None):
    ...
    ret = {"scores": scores, "clses": clses.float(), "xs": xs0, "ys": ys0, "cts": cts}
    ...
    if "ltrb" in output:
        ...
        bboxes = torch.cat(
            [
                xs0.view(batch, K, 1) + ltrb[..., 0:1],
                ys0.view(batch, K, 1) + ltrb[..., 1:2],
                xs0.view(batch, K, 1) + ltrb[..., 2:3],
                ys0.view(batch, K, 1) + ltrb[..., 3:4],
            ],
            dim=2,
        )
        ret["bboxes"] = bboxes
    ...
    for head in regression_heads:
        if head in output:
            ret[head] = tranpose_and_gather_feat(output[head], inds).view(batch, K, -1)

    return ret


def translate_centre_points(points, img_ctr, scale, out_size, BN, max_objects, rot=0):

    trans = torch.Tensor(
        get_affine_transform(img_ctr, scale, rot, out_size, inv=1).astype(np.float32)
    ).to("cuda")

    points = torch.cat(
        (points.reshape(BN, max_objects, 2), torch.ones(BN, max_objects, 1).to("cuda")),
        2,
    )
...
def match_predictions_ground_truth(predicted_centers, gt_centers, gt_mask, cams):
    ...
    cost_matrix = (
        torch.cdist(predicted_centers, gt_centers, p=2.0).cpu().detach().numpy()
    )

    for batch in range(num_preds[0]):
        mask = gt_mask[batch, cams[batch]].cpu().detach().numpy()
        cost_matrix[batch, :] = (
            cost_matrix[batch, :] * mask + np.bitwise_not(mask) * init_value
        )
...
def test_post_process(model_output, calib):
    ...
    rotation_offset = torch.atan((ct[:, 0] - cam_center_x) / focal_length_x)
...
def compare_ground_truth(
    detection_attributes, annotated_objects, image, calib, cam, opt
):
    ...
    images, bev, predictions, ground_truths = draw_results(
        [np.array(image.cpu())[0]], [calib], predicted_objects, annotated_objects
    )

    annotation_attributes = objects_to_attribute_list(ground_truths)
    prediction_attributes = objects_to_attribute_list(predictions)

    prediction_attributes["dd_bb_image"] = np.squeeze(
        np.array(prediction_attributes["dd_bb_image"]), -2
    )
    annotation_attributes["dd_bb_image"] = np.squeeze(
        np.array(annotation_attributes["dd_bb_image"]), -2
    )
    ...
    for i in range(len(gt_idx)):
        ...
        matching_stats = compare_pred_gt(
            predicted_objects[pred_idx[i]], annotated_objects[gt_idx[i]], cam
        )
    ...
    for key, val in prediction.items():
        if key in ground_truth:
            if key == "location":
                ...
                gt_vol = (
                    ground_truth[key][0] * ground_truth[key][1] * ground_truth[key][2]
                )
                ...
                stats[key] = abs(
                    ((((val - ground_truth[key]) * 180 / math.pi) + 180) % 360) - 180
                )
            elif key == "ddd_bb_world":
                stats["3D_iou"], stats["bev_iou"] = calculate_3D_iou(
