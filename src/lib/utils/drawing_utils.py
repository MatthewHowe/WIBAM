...
def draw_results(
    ...
    for pr in predictions:
        draw_object(
            pr, images, calibrations, draw, pr_colours[predictions.index(pr)], birds_eye
        )
    ...
    if "gt" in draw:
        for gt in ground_truths:
            draw_object(
                ...
                calibrations,
                draw,
                gt_colours[ground_truths.index(gt)],
                birds_eye,
            )
...
def draw_object(obj, images, calibrations, draw, colour, birds_eye):
    ...
    if "birds_eye" in draw and "location" in obj:
        if "ddd_bb_world" not in obj:
            obj["ddd_bb_world"], obj["ddd_bb_image"] = object_to_ddd_bb(
                obj, calibrations
            )
    ...
    for img in range(len(images)):
        ...
        for face_index in range(3, -1, -1):
            ...
            for i in range(4):
                ...
                cv2.line(
                    image,
                    (corners[face[i], 0], corners[face[i], 1]),
...
def draw_dd_box(dd_bb, images, colour=(255, 0, 0)):
    for img in range(len(images)):
        ...
        box = dd_bb[img]
        cv2.rectangle(
            image, (box[0], box[1]), (box[0] + box[2], box[1] + box[3]), colour, 2
        )
...
def draw_birds_eye(obj, birds_eye, colour=(0, 255, 0)):
    ...
    ddd_bb_bev = (obj["ddd_bb_world"][:4, :2] + birds_eye["offset"]) * birds_eye[
        "scale"
    ]
    arrow = np.array(
        [
    ...
    bev_point = tuple(
        ((ground_point + birds_eye["offset"]) * birds_eye["scale"]).astype(int)
    )
    cv2.circle(overlay, bev_point, 2, colour, -1)
    cv2.fillPoly(overlay, [ddd_bb_bev], colour, 1)

    alpha = 0.6
    cv2.addWeighted(
        overlay, alpha, birds_eye["image"], 1 - alpha, 0, birds_eye["image"]
    )
    cv2.polylines(birds_eye["image"], [ddd_bb_bev, arrow], True, colour, 1)
