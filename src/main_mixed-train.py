...
def main(opt):
  ...
  if opt.mixed_learning:
    mixed_dataset = get_dataset(opt.mixed_dataset)
  ...
  if opt.mixed_learning:
    mixed_writer = SummaryWriter(tensorboard_dir)
    mixed_total_writer = SummaryWriter(tensorboard_dir)
  ...
  if opt.mixed_learning:
    mixed_loader = torch.utils.data.DataLoader(
      mixed_dataset(opt, 'train'), batch_size=batch_size, shuffle=True,
  ...
  if opt.mixed_learning:
    opt.mixed_schedule = tuple(map(int,opt.mixed_schedule.split(',')))
    training_sets = math.ceil(opt.num_epochs/sum(list(opt.mixed_schedule)))
  else:
    training_sets = opt.num_epochs
  epoch = 0
  finished = False
